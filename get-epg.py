...
def getChannelEPG(fhandle, channelID):
    ...
    for n in range(len(channelID)):
        ...
        for detail2 in program2:
            #write programe
            st = (datetime.fromtimestamp(detail2['st'], timezone.utc) + timedelta(hours=0)).strftime('%Y%m%d%H%M%S')
        ...
        for detail3 in program3:
            #write programe
            st = (datetime.fromtimestamp(detail3['st'], timezone.utc) + timedelta(hours=0)).strftime('%Y%m%d%H%M%S')
